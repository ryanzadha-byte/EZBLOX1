...
local Window = Fluent:CreateWindow({Title = "❤️EZBLOX❤️",SubTitle = "| By:EZBLOX ",TabWidth = 180,Size = UDim2.fromOffset(660, 440),Acrylic = false,Theme = "Rose",MinimizeKey = Enum.KeyCode.End})
            ...
            elseif tonumber(replicated.Remotes.CommF_:InvokeServer("CDKQuest","Progress")["Evil"]) == -5 then
              ...
              if workspace.Map:FindFirstChild("HellDimension") then
                if (Root.Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 1000 then
                  for gg,BLOX in pairs(workspace.Map.HellDimension.Exit:GetChildren()) do
